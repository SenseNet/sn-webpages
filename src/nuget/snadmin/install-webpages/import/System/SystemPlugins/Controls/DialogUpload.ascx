...
<asp:Panel ID="Container" runat="server">
    <div>
        <asp:Button ID="UploadButton" runat="server" Text="<%$ Resources:Controls,Upload %>" onclientclick="SN.DialogUpload.openUploadDialog($(this));return false;" CssClass="sn-button sn-notdisabled" />

        <div class="sn-du-uploadloading" style="display:none;"><%=GetGlobalResourceObject("Controls", "LoadingLinks")%></div>
    ...
    <asp:Panel ID="UploadDialog" runat="server" style="padding:0; overflow:hidden;" >
        ...
        <div class="sn-sharecontent-buttoncontainer" style="margin-top:0;">
            <div class="sn-sharecontent-buttondiv">
                <input type="button" class="sn-submit sn-button sn-notdisabled" value="<%=GetGlobalResourceObject("Controls", "Close")%>" onclick="SN.DialogUpload.closeUploadDialog($(this));return false;" />
            </div>
        </div>
