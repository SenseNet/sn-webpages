    ...
    <xsl:for-each select="Column">
      <![CDATA[<sn:ListHeaderCell runat="server" class="sn-lg-col-]]><xsl:value-of select="position()"/><xsl:if test="$groupBy != '' and contains(@fullName,$groupBy)"><xsl:text> sn-lg-col-groupby</xsl:text></xsl:if><![CDATA[ sn-nowrap ui-state-default" FieldName="]]><xsl:value-of select="@fullName"/><![CDATA[" ]]><xsl:if test="@width > 0"><![CDATA[Width="]]><xsl:value-of select="@width"/><![CDATA["]]></xsl:if><![CDATA[>      
        <asp:LinkButton runat="server" CommandName="Sort" CommandArgument="]]><xsl:value-of select="@fullName"/><![CDATA[" >
          ...
          <span><%= ListHelper.GetColumnTitle("]]><xsl:value-of select="@fullName"/><![CDATA[", this.ContextNode) %></span>
        </asp:LinkButton>
      </sn:ListHeaderCell>]]>
    </xsl:for-each>
  </xsl:template>
    ...
    <xsl:for-each select="Column">
      <xsl:choose>
        <xsl:when test="contains(@modifiers,'main')">
          ...
          <![CDATA[ <sn:ActionMenu NodePath='<%# Eval("Path") %>' runat="server" Scenario="]]><xsl:value-of select="$mainscenario"/><![CDATA[" IconName="<%# ((SenseNet.ContentRepository.Content)Container.DataItem).Icon %>" OverlayVisible="true" >]]>
          <![CDATA[  <sn:ActionLinkButton runat='server' NodePath='<%# Eval("Path") %>' ActionName='<%# ListHelper.GetMainActionName((SenseNet.ContentRepository.Content)Container.DataItem) %>' IconVisible='false' >]]>
